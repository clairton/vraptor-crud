...
public abstract class AbstractController<T extends Model> {
    ...
    public AbstractController(final Class<? extends Model> modelType, final Repository repository,
            final Result result, @Language final Inflector inflector, final Mirror mirror,
    ...
    public @ExceptionVerifier @Get void index() {
        ...
        if (params != null && params.containsKey(Param.PAGE) && params.containsKey(Param.PER_PAGE)) {
        ...
        final Collection<Predicate> predicates = queryParser.parse(request, modelType);
        final Collection<?> collection = repository.from(modelType).where(predicates).collection(page, perPage);
    ...
    @Consumes(value = "application/json")
    public @ExceptionVerifier @Put("{id}") void update(final Long id, final T model) {
