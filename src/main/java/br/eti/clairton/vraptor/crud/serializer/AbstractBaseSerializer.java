...
public abstract class AbstractBaseSerializer<T extends Base> extends GsonJpaSerializer<T> implements JsonSerializer<T>, JsonDeserializer<T>, Resourceable<T> {
	private static final long serialVersionUID = 1L;
	private final br.eti.clairton.jpa.serializer.Tagable<T> tagable;


	public AbstractBaseSerializer(final Inflector inflector, final EntityManager em) {
		this(new NodesProgramatic(), inflector, em);
	}
	
	public AbstractBaseSerializer(final Nodes nodes, final Inflector inflector, final EntityManager em) {
		super(nodes, em);
		this.tagable = new Tagable<T>(inflector){
			...
			public String getRootTag(final T src) {
				return AbstractBaseSerializer.this.getRootTag(src);
			}

			@Override
			public String getResource(final T src) {
				return  AbstractBaseSerializer.this.getResource(src);
			}
